...
import os, re, sys, json, csv, io, hashlib, hmac, secrets
...
class Database:
    ...
    # All of this app's tables live in the PG_SCHEMA schema so they can
    # NEVER collide with other apps sharing the same database
    # (e.g. the Calendar App 2026 project's public.events table).
    PG_SCHEMA = 'cub_scouts'
    APP_TABLES = ('scouts', 'awards', 'events', 'signups', 'settings',
                  'event_slots', 'slot_signups', 'award_requirements', 'users')

    def _qualify(self, sql):
        """Prefix bare table names with the app schema (PostgreSQL only)."""
        for t in self.APP_TABLES:
            sql = re.sub(
                r'\b(FROM|JOIN|INTO|UPDATE|REFERENCES|TABLE IF NOT EXISTS|TABLE)(\s+)(' + t + r')\b',
                r'\1\2' + self.PG_SCHEMA + r'.\3',
...
POSTGRES_SCHEMA = ["""
    CREATE SCHEMA IF NOT EXISTS cub_scouts
""", """
    CREATE TABLE IF NOT EXISTS scouts (
